...
class Student
{
    ...
    void display()
    {
        String join = ""+doj.get(Calendar.DATE)+"/"+doj.get(Calendar.MONTH)+"/"+doj.get(Calendar.YEAR); Calendar.
        System.out.println("\n\nThe Details are: \nRegistration Number: "+String.format("%02d",reg_no)+"\nName: "+name+"\nSemester: "+sem+"\nGPA: "
    ...
    static void initials(Student[] s, int n)   
    {
        ...
        for(int i=0; i<n; i++)
        {
            ...
            String t = s[i].name;
            for(int j=0, k=0; j<t.length() && k<3; j++)
                if(t.charAt(j)==' ' && t.charAt(j+1)!=' ')
                    x[k++]=j+1;
            s[i].name = "" + Character.toUpperCase(t.charAt(x[0])) + ". " + Character.toUpperCase(t.charAt(x[1])) + ". " + t.substring(x[2]);
            System.out.println(s[i].name);
        }   
    }   
}
...
class one
{
    public static void main(String args[]) 
    {
        ...
        for(i=0; i<n; i++)
        {
            ...
            sc.nextLine();
            s[i]= new Student( sc.nextLine(), sc.nextInt(), sc.nextDouble(), sc.nextDouble(), new GregorianCalendar(sc.nextInt(), sc.nextInt(), sc.nextInt()) );
        }
